  ...
  while (remaining.length) {
    ...
    for (var i = 0; i < remaining.length; i++) {
      ...
      ready = _.chain(nextTask.depends)
        .map(function(task) { return _.some(completed, task); })
        .every()
        .value();
      ...
      if (ready) {
        ...
        // if theres no empty slots available then just
        // reset it
        if (available == -1) {
          if (resetIndex == -1 || resetIndex == workers) {
            resetIndex = 0
          }
          var lastIndex = w[resetIndex].length - 1
          w[resetIndex].push({
            start: w[resetIndex][lastIndex].finish,
            finish: w[resetIndex][lastIndex].finish + nextTask.cost,
            task: nextTask
          });
